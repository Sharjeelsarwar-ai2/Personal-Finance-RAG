...
def _status_badge(dot_color: str, label: str) -> str:
    return (
        f'<span style="display:inline-flex;align-items:center;gap:7px;padding:7px 10px;'
        f'border-radius:999px;color:#aeb9cb;background:rgba(255,255,255,.04);'
        f'border:1px solid rgba(255,255,255,.08);font-size:.63rem;font-weight:700;'
        f'letter-spacing:.03em;white-space:nowrap;">'
        f'<span style="width:6px;height:6px;border-radius:50%;background:{dot_color};'
        f'box-shadow:0 0 9px {dot_color};"></span>{label}</span>'
    )
...
def render_settings_panel() -> None:
    ...
    with st.container(border=True):
        ...
        with col_a:
            api_key = st.text_input("Groq API key", type="password", value=st.session_state.groq_api_key, placeholder="Paste your API key", help="Required to process documents and answer questions.")
...
def render_data_tab() -> None:
    ...
    with upload_col:
        with st.container(border=True):
            ...
            uploaded_files = st.file_uploader("Choose documents", type=["pdf", "csv", "xlsx", "txt", "docx"], accept_multiple_files=True, label_visibility="collapsed")
            if uploaded_files:
